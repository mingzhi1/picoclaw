...
// checkToolchain probes for common tools that picoclaw skills/tools may depend on.
func checkToolchain() {
	tools := []struct {
		name    string
		cmd     string
...
func checkModelsEmpty(cfg *config.Config) {
	if cfg.HasProvidersConfig() {
		...
		fmt.Println("   → Add a model: picoclaw chat \"配置 deepseek\"")
	}
}
...
func checkChannels(cfg *config.Config) {
	...
	if len(enabledChannels) == 0 {
		...
		fmt.Println("   → Enable one: picoclaw chat \"配置 telegram\"")
		return
	}
...
func checkGatewayPort(cfg *config.Config) {
	...
	if err != nil {
		...
		fmt.Println("   → Another picoclaw instance running? Or change gateway.port")
	}
}
