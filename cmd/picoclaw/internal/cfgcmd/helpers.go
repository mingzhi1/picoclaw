...
func newGetCommand() *cobra.Command {
	return &cobra.Command{
		...
		Example: `  picoclaw config get agents.defaults.primary_model
  picoclaw config get gateway.port
  picoclaw config get channels.telegram.enabled`,
...
func newSetCommand() *cobra.Command {
	return &cobra.Command{
		...
		Example: `  picoclaw config set python               # auto-detect python path
  picoclaw config set uv                   # auto-detect uv path
  picoclaw config set npm                  # auto-detect npm path
  picoclaw config set agents.defaults.primary_model deepseek-chat
  picoclaw config set gateway.port 8080`,
		RunE: func(_ *cobra.Command, args []string) error {
			if len(args) == 1 {
				...
				return fmt.Errorf("unknown tool %q — known tools: python, uv, npm, node, git, curl\n"+
					"For key=value: picoclaw config set <key> <value>", args[0])
			}
...
func runSetTool(name string) error {
	...
	fmt.Printf("   picoclaw config set tools.env.%s /path/to/%s\n", name, name)
	return nil
}
