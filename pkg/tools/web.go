...
// createHTTPClient creates an HTTP client with optional per-tool proxy override.
// If proxyURL is empty, the global proxy (from httpclient.Init) is used.
func createHTTPClient(proxyURL string, timeout time.Duration) (*http.Client, error) {
	return httpclient.NewWithProxy(timeout, proxyURL)
}
