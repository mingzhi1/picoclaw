...
MetaClaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, MetaClaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `METACLAW_TOOLS_<SECTION>_<KEY>`.

Legacy `PICOCLAW_*` environment variables are still accepted for backward compatibility.

For example:

- `METACLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `METACLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `METACLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `METACLAW_TOOLS_MCP_ENABLED=true`
